...
function StatsSection() {
  ...
  useEffect(() => {
    // Fetch completed auctions count
    ...
    // Fetch personal users (registered bidders)
    fetch("http://localhost:3000/user/getusers")
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (!data) return;
        const list = Array.isArray(data) ? data : data.data ?? data.users ?? [];
        const bidders = list.filter(u => u.role === "personal" || u.userType === "personal").length;
  ...
  const stats = [
    ...
    { value: 94,             label: "Lakh in Sales",      prefix: "₹", suffix: "+", icon: Icons.TrendUp(28) },
    { value: 75,             label: "Satisfaction Rate",  suffix: "%", icon: Icons.Star(28) },
  ];

  return (
    <div style={{ background: t.bgSec, borderTop: `1px solid ${t.border}`, borderBottom: `1px solid ${t.border}`, padding: "60px 40px" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "32px" }}>
        {stats.map((s, i) => (
          <div key={i} style={{ textAlign: "center" }}>
            ...
            <div style={{ fontSize: "40px", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1, background: "linear-gradient(135deg, #38bdf8, #818cf8)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              {s.value === null
...
function TrustSection() {
  ...
  return (
    <div style={{ background: t.bgSec, padding: "60px 40px" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "24px" }}>
          {badges.map((b, i) => (
            <div key={i} style={{ padding: "28px 24px", background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: "16px", textAlign: "center", boxShadow: t.shadowCard }}>
...
export const HomeComponent = () => {
  ...
  const liveAuctions = useMemo(
    () => shuffleItems(auctions.filter((a) => a.status === "Active")).slice(0, 5),
    [auctions]
  );

  const featuredAuctions = useMemo(
    () => shuffleItems(auctions.filter((a) => a.status === "Scheduled")).slice(0, 5),
    [auctions]
  );
  ...
  const renderCards = (items, fallbackCount = 5) => {
  ...
  return (
    <div style={{ background: t.bg, minHeight: "100vh", fontFamily: "'Segoe UI', system-ui, sans-serif", transition: "background 0.25s" }}>
      ...
      {/* ── LIVE AUCTIONS ── */}
      <div style={{ background: t.bg, padding: "72px 40px", borderTop: `1px solid ${t.L ? "rgba(244,63,94,0.08)" : "rgba(244,63,94,0.12)"}` }}>
      ...
      {/* ── FEATURED ── */}
      <div style={{ background: t.bgSec, padding: "72px 40px" }}>
        <div style={{ maxWidth: "1600px", margin: "0 auto" }}>
      ...
      {/* ── HOW IT WORKS ── */}
      <div style={{ background: t.bgDark, padding: "80px 40px", borderTop: `1px solid ${t.border}` }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "56px" }}>
            ...
            <h2 style={{ color: t.textPri, fontSize: "36px", fontWeight: 800, margin: 0 }}>How It Works</h2>
          </div>
      ...
      {/* ── CTA BANNER ── */}
      <div style={{ padding: "80px 40px", background: "linear-gradient(135deg, #0ea5e9 0%, #38bdf8 100%)", textAlign: "center" }}>
        <h2 style={{ color: "white", fontSize: "40px", fontWeight: 900, margin: "0 0 16px" }}>Ready to start bidding?</h2>
        <p style={{ color: "rgba(255,255,255,0.8)", fontSize: "18px", marginBottom: "36px" }}>Join as bidders and sellers — it's completely free.</p>
